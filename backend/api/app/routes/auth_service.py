...
def login(
    ...
    # ── Return the token BOTH ways ────────────────────────────────────────────
    #
    # 1. HttpOnly cookie  → used by same-origin and lax same-site requests.
    #    The browser sends this automatically; JS cannot read it (secure).
    #
    # 2. JSON body field "access_token"  → read by the frontend and kept in
    #    memory via setApiToken().  Used as Authorization: Bearer <token> on
    #    cross-origin PATCH / DELETE requests where samesite="lax" prevents
    #    the cookie from being forwarded during local development.
    #    The token is never written to localStorage or sessionStorage.
    #
    # Both point to the same JWT with the same expiry.  This is a standard
    # pattern for SPAs that need to work across origins during development
    # without weakening production cookie security.

    response = JSONResponse(
        content={
            "message": "Login exitoso",
            "access_token": token,          # ← Bearer fallback for frontend
            "worker_id": str(user_info["worker_id"]),
            "rank_level": user_info["rank_level"],
            "rank_name": user_info["rank_name"],
        }
