...
class TestWorkerEndpoints:
    ...
    def test_get_worker_details_success(self, client, db):
        ...
        response = client.get(f"/worker/{worker.id}/details")

        assert response.status_code == 200
        assert response.json()["rank"] == "Lider"
        assert response.json()["group"] == "Grupo A"

    def test_update_worker_flag_success(self, client, db, leader_user):
