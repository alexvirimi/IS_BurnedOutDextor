# app/tests/routes/auth_service_test.py
import uuid
...
class TestAuthEndpoints:
    ...
    def test_register_success(self, client, db):
        ...
        area = Area(id=uuid.uuid4(), name="Tecnología")
        ...
        response = client.post(
            "/auth/register",
            data={"worker_id": str(worker.id), "username": "mario", "password": "1234"}
        )
    ...
    def test_register_worker_not_found(self, client):
        response = client.post(
            "/auth/register",
            data={"worker_id": str(uuid.uuid4()), "username": "fake", "password": "1234"}
        )
    ...
    def test_register_username_exists(self, client, db):
        ...
        auth_user = AuthUser(id=uuid.uuid4(), worker_id=worker.id, username="ana", password=hashed)
        db.add_all([rank, area, group, worker, auth_user])
        db.commit()

        response = client.post(
            "/auth/register",
            data={"worker_id": str(worker.id), "username": "ana", "password": "nueva"}
        )
    ...
    def test_login_success(self, client, db):
        ...
        import bcrypt
        hashed = bcrypt.hashpw(b"1234", bcrypt.gensalt()).decode()
        auth_user = AuthUser(id=uuid.uuid4(), worker_id=worker.id, username="luis", password=hashed)
        db.add_all([rank, area, group, worker, auth_user])
        db.commit()

        response = client.post(
            "/auth/login",
            data={"username": "luis", "password": "1234"}
        )
